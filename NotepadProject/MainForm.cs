...
using NotepadCore.Services;
using NotepadCore;
using NotepadProject.Forms;
...
namespace NotepadProject
{
    public partial class MainForm : Form
    {
        ...
        private void newFileMenu_Click(object sender, System.EventArgs e)
        {
            ...
            {
                //SingleLinkedList MyFirstList = new SingleLinkedList();
                //SingleLinkedList MyFirstList2 = new SingleLinkedList();
                ...
                if (result == DialogResult.Yes)
                {
                    if (fileOperation.Filename.Contains("Untitled"))
                    {
                        ...
                        {
                            string txt = txtArea.Text.ToString();


                            //string c = txt.Substring(0, 1);

                            ////singleLinkedlist node1 = new singleLinkedlist(c);

                            string[] storevalue = new string[txt.Length];

                            for (int i = 0; i < txtArea.Lines.Length; i++)
                            {
                                my_list.AddLast(txtArea.Lines[i].ToString());



                            }
                            //copy all traversing values of linkedlist to string array
                          
                            my_list.CopyTo(storevalue, 0);

                            finalvalues = storevalue;
                            //inserting values in textfile and save it


                            fileOperation.SaveFile(newFileSave.FileName, finalvalues);
                            UpdateView();
                    ...
                    {
                        fileOperation.SaveFile(fileOperation.FileLocation, finalvalues);
                        UpdateView();
                    }
        ...
        private void undoEditMenu_Click(object sender, EventArgs e)
        {
            ...
            //LoginInfo.userID = txtArea.Text;
            //txtArea.Text = DateTime.Now.ToString() + txtArea.Text;
            //MessageBox.Show(txtArea.Text);
            //EditOperation editOperation1 = new EditOperation();

            //editOperation.ClearStacks();
            txtArea.Text = editOperation.UndoClicked();
        ...
        private void UpdateStatus()
        {
            ...
            //int line = txtArea.GetLineFromCharIndex(pos) ;
            ...
            int pos = txtArea.SelectionStart;
            int line = txtArea.GetLineFromCharIndex(pos);
            int column = txtArea.SelectionStart - txtArea.GetFirstCharIndexFromLine(line);
            status.Text = "Line " + (line + 1) + ", Column " + (column + 1);
        }
